...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=int(cfg['reduce_n_step']), gamma=float(cfg['reduce_lr_coeff']))
...
for i in range(cfg['n_iter']):
    ...
    if i % int(cfg['log_interval'])==0:
        ...
        scheduler.step()
        if XP_type=='MNIST':
            acc.append(((u[:, :n_func_pred].flatten().detach().cpu() == torch.argmax(out[:, :n_func_pred],dim=-1).flatten().detach().cpu()).sum() / (n_func_pred * N)).item())
            ...
            print(f'| Iter {i:3d} | time: {elapsed:5.2f}s  | Loss : {loss:3.6f} | MSE: {MSE:3.6f} | RMSE: {RMSE:3.6f} |  ')
